...
if(@$_GET['c']=='success'){
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        if(strlen($data['adminName'])<6 || strlen($data['adminPwd'])<6)die("<script>alert('管理员账号密码长度不可以低于6');history.go(-1)</script>");
        // 创建数据库并选中
        ...
        // 导入sql数据并创建表
        $bjyblog_str=file_get_contents('./test.sql');
        $bjyblog_str=str_replace('yz_admin_username',$data['adminName'],$bjyblog_str);
        $bjyblog_str=str_replace('yz_admin_password',md5(md5($data['adminPwd'])),$bjyblog_str);
        $sql_array=preg_split("/;[\r\n]+/", str_replace('yy_',$data['DB_PREFIX'],$bjyblog_str));
